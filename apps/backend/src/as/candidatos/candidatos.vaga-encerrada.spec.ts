...
function makeDb(cenario: {
  ...
  const select = vi.fn(() => {
    ...
    // A PONTE A&S → Esteira lê o candidato (CPF + folha da vaga) antes de consumir a posição: é o
    // único `select` deste fake resolvido por `then`. Devolve um candidato com CPF VÁLIDO, para o
    // gesto de envio passar do gate de CPF e chegar à conferência de status da vaga, que é o que
    // este arquivo mede. Sem isto, o gate de CPF recusaria antes, e o teste do encerramento não
    // chegaria a exercer o status.
