...
import { CilindroMeta, percentualDoCilindro } from "@/components/ui/CilindroMeta";
...
import { rotuloDaLinhaPorId, useLinhasServico } from "@/lib/as-linhas-servico";
...
/* A TRILHA SAIU DESTE ARQUIVO (frente do reuso pela Liberar Vaga). O que ficou aqui importado dela
   ...
   definição só, e não uma cópia por tela. */
import {
  Campo,
  TrilhaDaVaga,
  formatCpf,
  ...
  type ModoDaTrilha,
  type Opcoes,
} from "@/components/as/vagas/TrilhaDaVaga";
...
/*
 * A CONTAGEM DE POSIÇÕES PREENCHIDAS SAIU DAQUI e mora em `@/lib/as-vagas-ocupacao` (etapa 3 da
 * tela unificada de vagas). O `preenchidas` continua sendo a peça única que a coluna lê, e o que
 * mudou foi o endereço: a régua precisa de teste de unidade, e testá-la aqui dentro obrigaria a
 * montar a página inteira. A régua em si (vaga viva lê a alocação de agora, vaga encerrada mantém o
 * número do fechamento, banco continua no número digitado) está documentada no topo daquele arquivo.
 */

/**
 * ─ O CILINDRO DE UMA META: O DESENHO SAIU DAQUI e virou peça compartilhada ────────────────────
 *
 * Ele nasceu nesta tela (OST de 27/08) e agora tem um SEGUNDO consumidor, o Gerenciador Do Portal.
 * O componente mora em `@/components/ui/CilindroMeta` e o desenho é o MESMO, classe por classe:
 * esta tela não mudou um pixel (§A.26). O que ficou aqui é só a FRASE do `title`, que fala de
 * origem de contagem (derivada, fechamento, ausente) e é vocabulário de vaga, não de portal.
 *
 * O que o cilindro diz continua documentado no componente: número no fim da barra, meta atingida
 * em verde com o nome dentro, meta zero sem barra e meta nula com "não informado" (§A.11).
 */
function tituloDoCilindro(
  rotulo: string,
  meta: number,
  feitas: number,
  origem: OrigemContagem,
): string {
  const pct = percentualDoCilindro(meta, feitas);
  if (meta === 0) return `${rotulo}: esta vaga não reservou posições.`;
  if (origem === "derivada")
    return `${rotulo}: ${feitas} de ${meta} preenchidas, ${pct}% da meta. A conta acompanha as alocações desta vaga agora.`;
  if (origem === "fechamento")
    return `${rotulo}: ${feitas} de ${meta} preenchidas, ${pct}% da meta. O número foi contado no fechamento da vaga e não muda mais.`;
  return `${rotulo}: meta de ${meta}. A contagem de preenchimento ainda não existe para esta vaga.`;
}

/**
 * O CILINDRO DESTA TELA: o componente compartilhado, com o `title` que só a Central de Vagas sabe
 * escrever. A prop `rotuloCheia` NÃO é passada de propósito: o padrão dela é "Meta Atingida", que
 * é exatamente o que esta tabela desenha hoje.
 */
function CilindroDaVaga({
  rotulo,
  meta,
  feitas,
  origem,
  ...
  feitas: number;
  /**
   * DE ONDE VEIO O NÚMERO (`@/lib/as-vagas-ocupacao`), que é o que o `title` precisa dizer: a vaga
   * viva acompanha as alocações de agora, a encerrada mostra o que foi contado no fechamento, e
   * `ausente` é a barra vazia de quem nunca teve contagem nenhuma, que não é a mesma barra vazia de
   * quem contou zero.
   */
  origem: OrigemContagem;
}) {
  return (
    <CilindroMeta
      ...
      meta={meta}
      feitas={feitas}
      title={meta === null ? undefined : tituloDoCilindro(rotulo, meta, feitas, origem)}
    />
  );
}
...
/*
 * O `STATUS_ENCERRADOS` VEM DE `@/lib/as-vagas-ocupacao`, importado no topo com o mesmo nome de
 * antes. A lista saiu daqui porque a régua de posições preenchidas passou a depender dela também, e
 * duas cópias do que é "vaga encerrada" divergiriam no dia em que um status novo aparecesse. Quem
...
export default function CentralDeVagasPage() {
  ...
  /**
   * ─ A TRILHA DE ABERTURA, QUE AGORA É UM MODO E NÃO UM PUNHADO DE FLAGS ───────────────────────
   *
   * Nulo é trilha fechada. Com valor, o componente `TrilhaDaVaga` é MONTADO, e todo o estado do
   * formulário (os 38 campos, o passo, os benefícios, os testes, as pendências) vive lá dentro.
   *
   * A PÁGINA CONTINUA DONA DOS CATÁLOGOS que ela já lia para os próprios filtros (`opcoes`,
   * `contexto`, segmentos, as listas de cliente e de cargo) e só os repassa: nenhuma busca nova.
   */
  const [trilha, setTrilha] = useState<ModoDaTrilha | null>(null);

  /**
   * ─ A BUSCA DO DETALHE DA VAGA AO ABRIR A TRILHA (correção de LGPD ativo, 22/09/2026) ──────────
   *
   * O `substituidoCpf` SAIU da lista (`VagaListItem` não o carrega mais): ele descia CRU para todo
   * consultor a cada carga, e a régua da §A.6 é minimização. Quem abre a trilha para EDITAR um
   * rascunho ou CLONAR uma vaga precisa do CPF para preencher o campo, então a tela busca UMA vaga
   * por vez, por `GET /as/vagas/:id`, no momento do clique, e nunca a lista inteira.
   *
   * `carregandoTrilha` desenha um aviso de espera enquanto o detalhe não chega, e `erroDetalhe` uma
   * mensagem se a busca falhar: em nenhum dos dois a trilha abre com CPF velho nem com a tela branca.
   */
  ...
  /**
   * O CPF DO SUBSTITUÍDO NA FICHA DO PAINEL, buscado à parte (mesma correção de LGPD ativo). A lista
   * não traz mais o CPF, então quando o painel abre uma vaga com motivo "Substituição" a tela o busca
   * por `GET /as/vagas/:id`, uma vez. "carregando" enquanto não chega, "erro" se a busca falhar: a
   * ficha nunca mostra CPF de outra vaga nem quebra, e o número some junto com o fechamento do painel.
   */
  const [cpfSubstituido, setCpfSubstituido] = useState<{
  ...
  /**
   * A OUTRA RECUSA DO MESMO BOTÃO: posição OFICIAL ainda aberta.
   *
   * ELA VIVE EM ESTADO PRÓPRIO, e não dentro do `erroFechar`, porque não é uma frase: é um corpo com
   * números e com a resposta de quem pode forçar. Enquanto ela caiu no `erroFechar`, o consultor lia
   * a recusa como erro seco e não tinha para onde ir, e o Master não tinha como encerrar assim mesmo:
   * a Etapa 6 inteira não tinha caminho na tela.
   */
  const [recusaFech, setRecusaFech] = useState<FecharVagaRecusa | null>(null);
  ...
  /**
   * O AVISO DA REDUÇÃO DE META, guardado com os números que ele já validou.
   *
   * ELE NÃO BLOQUEIA: a decisão continua sendo do consultor, e a confirmação salva. O que ele impede
   * é o gesto ser SILENCIOSO, porque baixar a meta até o número já entregue faz a vaga fechar pela
   * porta normal, sem Master e sem a trilha do forçamento (achado da auditoria, decisão do diretor:
   * rastro, não trava).
   *
   * OS NÚMEROS VIAJAM JUNTO DO TEXTO em vez de serem relidos do formulário na confirmação: o par que
   * o diálogo descreve tem de ser exatamente o par que vai ao servidor, senão a frase que a pessoa
   * leu deixa de ser o que ela confirmou.
   */
  const [avisoReducao, setAvisoReducao] = useState<{
    texto: string;
    oficiais: number;
    banco: number;
  } | null>(null);
  ...
  /**
   * A VAGA ABERTA NO PAINEL ACOMPANHA A LISTA RELIDA (etapa 5).
   *
   * `verAlvo` guarda o OBJETO da linha, não o id, então depois de um `carregar()` ele seria o retrato
   * ANTIGO: a ação escrita dentro do painel atualizaria a tabela atrás e deixaria o cabeçalho do
   * próprio painel (a trilha, que lê a ocupação) dizendo o número de antes, na mesma tela e ao mesmo
   * tempo. Aqui ele é reapontado para a linha nova da lista.
   *
   * NÃO HÁ LAÇO: depois do `set`, `verAlvo` É o objeto vindo de `rows`, então a comparação por
   * referência falha na próxima passada e nada mais é gravado. E como a `key` do painel continua
   * sendo o id, ele NÃO remonta: a aba escolhida e a ordenação da tabela ficam onde estavam.
   */
  useEffect(() => {
    if (!verAlvo) return;
    const fresca = rows.find((v) => v.id === verAlvo.id);
    if (fresca && fresca !== verAlvo) setVerAlvo(fresca);
  }, [rows, verAlvo]);

  /**
   * O CPF DO SUBSTITUÍDO CHEGA PELA ROTA DE DETALHE (correção de LGPD ativo). A lista não o traz
   * mais, então quando o painel abre uma vaga com motivo "Substituição" a tela o busca UMA vez, por
   * `GET /as/vagas/:id`. As dependências são o id e o motivo: reapontar `verAlvo` para a linha relida
   * (o efeito acima) não os muda, então o `carregar()` da tela NÃO refaz esta busca. Só o CPF vem por
   * aqui; todo o resto da ficha continua saindo do item da lista, que já está na mão.
   */
  useEffect(() => {
    if (!verAlvo || verAlvo.motivo !== MOTIVO_SUBSTITUICAO) {
      setCpfSubstituido({ estado: "ok", cpf: null });
      return;
    }
    let vivo = true;
    setCpfSubstituido({ estado: "carregando", cpf: null });
    apiFetch<VagaDetalhe>(`/as/vagas/${verAlvo.id}`, { token })
      .then((d) => {
        if (vivo) setCpfSubstituido({ estado: "ok", cpf: d.substituidoCpf });
      })
      .catch(() => {
        if (vivo) setCpfSubstituido({ estado: "erro", cpf: null });
      });
    return () => {
      vivo = false;
    };
  }, [verAlvo?.id, verAlvo?.motivo, token]);

  /**
   * OS TRÊS PONTOS DE ENTRADA DA TRILHA, que agora são um `setTrilha` cada.
   *
   * O ESTADO DELA É O MODO, e não uma coleção de flags: abrir é montar o componente, fechar é
   * desmontá-lo. O que antes era zerar oito estados na mão virou o estado inicial da montagem.
   *
   * RASCUNHO E CLONE FECHAM O PAINEL DA VAGA antes de abrir a trilha, como já faziam: os dois são
   * clicados de dentro dele, e deixá-lo aberto por trás poria duas caixas na tela.
   */
   ...
   *
   * A DIFERENÇA PARA O CLONE, e é toda a diferença: aqui o CÓDIGO VOLTA e a vaga é marcada como em
   * edição, então o salvamento ATUALIZA aquela vaga em vez de criar outra. No clone o código nasce
   * vazio e uma vaga nova é criada, porque cada processo seletivo tem o número dele.
   */
  /**
   * BUSCA A VAGA COMPLETA (lista MAIS o `substituidoCpf`) para abrir a trilha, UMA por vez. Devolve
   * `null` quando a busca falha, e nesse caso quem chamou NÃO abre a trilha: o `erroDetalhe` já está
   * na tela, e abrir a trilha sem o detalhe seria preencher o CPF com o valor de outra vaga.
   */
  async function buscarDetalhe(id: string): Promise<VagaDetalhe | null> {
    setErroDetalhe(null);
    setCarregandoTrilha(true);
    try {
      return await apiFetch<VagaDetalhe>(`/as/vagas/${id}`, { token });
    } catch {
      setErroDetalhe("Não foi possível abrir a vaga. Tente de novo.");
      return null;
    } finally {
  ...
  async function continuarRascunho(v: VagaListItem) {
    const detalhe = await buscarDetalhe(v.id);
    if (!detalhe) return;
    setVerAlvo(null);
    setTrilha({ tipo: "rascunho", vaga: detalhe });
  }
   ...
   * fim da trilha, depois de a pessoa ter revisado os 37 outros campos.
   *
   ...
   */
  async function clonarVaga(v: VagaListItem) {
    const detalhe = await buscarDetalhe(v.id);
    if (!detalhe) return;
    setVerAlvo(null);
  ...
  async function confirmarPosicoes(e: FormEvent) {
    ...
    /**
     * ┌─ O AVISO ANTES DE CONFIRMAR, E SÓ NA REDUÇÃO ──────────────────────────────────────────┐
     * │ A auditoria provou que baixar a meta oficial até o número já entregue fecha a vaga pela  │
     * │ porta NORMAL, sem Master e sem a trilha do forçamento. O diretor decidiu RASTRO, não      │
     * │ trava: o consultor continua podendo baixar, e passa a saber que aquilo fica registrado.   │
     * │                                                                                          │
     * │ AUMENTAR NÃO AVISA NADA. Aumentar afasta o fechamento em vez de aproximá-lo, e avisar no  │
     * │ caso inofensivo ensinaria o time a fechar o diálogo sem ler, justamente o diálogo que      │
     * │ existe para ser lido.                                                                     │
     * └──────────────────────────────────────────────────────────────────────────────────────────┘
     *
     * A RÉGUA E O TEXTO MORAM EM `@/lib/as-vaga-meta`, com teste. Aqui fica só a decisão de abrir o
     * diálogo, e o nulo é o contrato: sem aviso, salva direto, como sempre salvou.
     */
    const aviso = avisoDeReducaoDeMeta(
      { oficiais: posAlvo.posicoesOficiais, banco: posAlvo.posicoesBanco },
      { oficiais, banco },
    );
    if (aviso) {
      setAvisoReducao({ texto: aviso, oficiais, banco });
      return;
    }

    await salvarPosicoes(oficiais, banco);
  }

  /**
   * O ENVIO DAS POSIÇÕES, separado do `submit` porque ele parte de DOIS lugares: o formulário, quando
   * não houve redução, e a confirmação do aviso, quando houve. O corpo é o mesmo par de números nos
   * dois caminhos.
   */
  async function salvarPosicoes(oficiais: number, banco: number) {
    if (!posAlvo) return;
    setSalvandoPos(true);
    try {
      ...
      // O DIÁLOGO FECHA NO ERRO e a mensagem aparece no formulário, que é onde a pessoa está
      // olhando: mantê-lo aberto por cima esconderia a frase que explica a recusa.
      setAvisoReducao(null);
   ...
   */
  async function enviarFechamento(opcoes?: { forcar?: boolean }) {
    ...
    try {
      await apiFetch(`/as/vagas/${fecharAlvo.id}/fechar`, {
        ...
        body: {
          ...
          /**
           * O `forcar` SÓ VIAJA QUANDO ALGUÉM FORÇOU, e nunca como `false` explícito: o fechamento
           * normal não é "um forçamento desligado", e mandar a chave em toda requisição faria a
           * exceção parecer parte do caminho comum de quem lê o tráfego depois.
           *
           * ELE NÃO É A AUTORIZAÇÃO, É O PEDIDO. Quem decide se um Master está do outro lado é o
           * service, que recalcula o papel quando esta chave chega e devolve 403 ao COMUM.
           */
      ...
      /**
       * A RECUSA POR POSIÇÃO OFICIAL ABERTA, CASADA PELO CAMPO `motivo` e nunca pela mensagem (a
       * régua e o parser moram em `@/lib/as-vaga-fechamento`, testados). Ela abre o modal que diz o
       * número real e, para quem pode, oferece o encerramento assim mesmo.
       *
       * O 403 NÃO CAI AQUI, E ISSO É DE PROPÓSITO: quando um COMUM chega a mandar `forcar` (tela
       * desatualizada, papel trocado no meio da sessão), o guard responde 403 e a frase dele aparece
       * no formulário, como qualquer outra recusa. A tela não tenta adivinhar o papel de novo.
       */
      const recusa = fechamentoRecusadoPorPosicoes(err);
      if (bloqueio) {
        setPendentesFech(bloqueio.pendentes);
        setRecusaFech(null);
        setErroFechar(null);
      } else if (recusa) {
        setRecusaFech(recusa);
        setErroFechar(null);
      } else {
        // Uma recusa nova fecha o modal do forçamento: deixá-lo aberto por cima esconderia a frase
        // que explica o que aconteceu de fato.
        setRecusaFech(null);
  ...
  const colunasOrdenaveis = useMemo<ColOrd<VagaListItem>[]>(
    () => [
       ...
       */
      {
        chave: "status",
        tipo: "status",
        valor: (v) => ordemDoStatusVaga(v.status, catalogoStatus),
      },
      /**
  ...
  return (
    <>
                    ...
                    <td>
                      ...
                      <div className="flex min-w-[104px] flex-col gap-1.5">
                        <CilindroDaVaga
                          ...
                          origem={origemContagem(v, "oficial")}
                        />
                        <CilindroDaVaga
                          rotulo="Banco"
      ...
      {/* ── TRILHA DE ABERTURA ────────────────────────────────────────────── */}
      {/* ELA MORA EM `components/as/vagas/TrilhaDaVaga`, e saiu daqui para poder ser REUSADA pela
          tela de Liberar Vaga, que vai passar a pedir a vaga inteira (§A.26: reusar, nunca
          duplicar). O QUE A CENTRAL DE VAGAS FAZ CONTINUA IGUAL: ela busca `opcoes`, `contexto` e o
          catálogo de segmentos como sempre buscou (os filtros dependem deles) e só os repassa.

          MONTADA SÓ QUANDO ABERTA, COM `key` NA VAGA: é a chave que faz o formulário nascer limpo a
          cada abertura, em vez de depender de alguém lembrar de zerar campo por campo. O tipo entra
          na chave porque clonar e continuar a MESMA vaga são duas trilhas diferentes. */}
      {trilha && (
        <TrilhaDaVaga
          key={`${trilha.tipo}:${"vaga" in trilha ? trilha.vaga.id : "nova"}`}
          modo={trilha}
          catalogos={{
            opcoes,
            contexto,
            segmentos: catalogoSegmentos,
            optClientes,
            optCargos,
      ...
      {/* ── A ESPERA E A FALHA DA BUSCA DO DETALHE AO ABRIR A TRILHA (correção de LGPD ativo) ──
          A trilha só monta com a vaga completa na mão. Entre o clique e a resposta de
          `GET /as/vagas/:id`, este aviso segura o olho; se a busca falhar, o modal de erro toma o
          lugar, e a trilha não abre com CPF de outra vaga. O aviso de espera não fecha por engano
          (§A.41): ele some sozinho quando o detalhe chega ou a busca falha. */}
      {carregandoTrilha && (
      ...
      {erroDetalhe && (
        <Modal onClose={() => setErroDetalhe(null)} ariaLabel="Erro ao abrir a vaga" className="max-w-sm">
          <div className="p-5">
            <p
            ...
            <div className="mt-4 flex justify-end">
              <Button variant="secondary" onClick={() => setErroDetalhe(null)}>
                Fechar
              </Button>
            </div>
          </div>
          ...
          <>
            ...
            <BlocoFicha titulo="Contratação">
              ...
              {verAlvo.motivo === MOTIVO_SUBSTITUICAO && (
                <>
                  ...
                  {/* Item 3: o CPF aparece COMPLETO e formatado (000.000.000-00) para leitura, e a
                        cópia da folha precisa dele inteiro. Ele NÃO vem mais da lista (correção de
                        LGPD ativo): é buscado à parte por `GET /as/vagas/:id` quando o painel abre,
                        e enquanto não chega a linha diz "carregando", nunca um CPF de outra vaga.
                        §A.6: a rota inteira do módulo é fechada pelo menu `as-vagas`, e o número
                        nunca vai para log. */}
                  <Linha
                    rotulo="CPF do substituído"
                    valor={
                      cpfSubstituido.estado === "carregando"
                        ? "carregando"
                        : cpfSubstituido.estado === "erro"
                          ? "não foi possível carregar o CPF"
                          : cpfSubstituido.cpf
                            ? formatCpf(cpfSubstituido.cpf)
                            : null
                    }
                  />
                </>
            ...
            <BlocoFicha titulo="Requisitos">
              ...
              <Linha
                ...
                valor={listaEmTexto(textoDosIdiomas(verAlvo), verAlvo.idiomasOutros, OPCAO_OUTROS)}
              />
      ...
      {/* ── O AVISO DA REDUÇÃO DE META ────────────────────────────────────
          TRANSPARÊNCIA, NÃO ACUSAÇÃO, e é por isso que ele é `warn` e não `danger`: o consultor
          TEM o direito de baixar a meta, e um diálogo vermelho transformaria uma operação normal em
          suspeita. Ele informa o de/para concreto e que a alteração fica registrada, e segue.

          E NÃO É `default`, que era o que estava aqui: o check azul do tom neutro anunciava SUCESSO
          num diálogo que ainda pergunta, e o ícone tem de seguir o estado real (§A.12). Atenção é
          exclamação. (Decisão do diretor, 09/09.)

          O RÓTULO DO BOTÃO É O MESMO DO FORMULÁRIO ("Salvar posições"): quem confirma está salvando
          o que já digitou, e um verbo novo aqui sugeriria uma segunda ação que não existe. */}
      <ConfirmDialog
        // O `posAlvo` ENTRA NA GUARDA para o aviso nunca sobreviver ao formulário que o gerou: sem
        // ele, fechar as posições com o diálogo aberto deixaria uma pergunta na tela sobre uma vaga
        // que não está mais em edição.
        open={posAlvo !== null && avisoReducao !== null}
        title="Reduzir A Meta De Posições?"
        message={avisoReducao?.texto ?? ""}
        confirmLabel="Salvar posições"
        cancelLabel="Cancelar"
        tone="warn"
        busy={salvandoPos}
        onConfirm={() => {
          if (avisoReducao) void salvarPosicoes(avisoReducao.oficiais, avisoReducao.banco);
        }}
        onCancel={() => setAvisoReducao(null)}
      />
      ...
      {fecharAlvo && !pendentesFech && !recusaFech && (
        <Modal
      ...
      {/* ── A VAGA QUE NÃO FECHA COM POSIÇÃO OFICIAL ABERTA ───────────────── */}
      {/* Mesmo desenho da fila de pendentes: o formulário sai da frente e o preenchimento FICA no
          estado, então cancelar aqui devolve o formulário como estava, sem redigitar nada. */}
      {fecharAlvo && recusaFech && (
        <RecusaFechamentoModal
          recusa={recusaFech}
          vagaRotulo={fecharAlvo.nomeDivulgacao ?? rotuloDaVaga(fecharAlvo)}
          forcando={fechando}
          onCancelar={() => setRecusaFech(null)}
          onForcar={() => void enviarFechamento({ forcar: true })}
        />
      )}
