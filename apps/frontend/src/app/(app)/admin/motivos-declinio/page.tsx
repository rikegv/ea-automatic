...
export default function MotivosDeclinioPage() {
  const { token } = useAuth();
  ...
  // motivo pendente de confirmação de inativação (null = modal fechado) + estado de "processando".
  const [confirmar, setConfirmar] = useState<Motivo | null>(null);
  const [inativando, setInativando] = useState(false);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      setRows(await apiFetch<Motivo[]>("/admin/motivos-declinio", { token }));
      setError(null);
    } catch (e) {
  ...
  function iniciarEdicao(c: Motivo) {
    setEditando(c.id);
    setNome(c.nome);
    setError(null);
  ...
  async function salvar(e: FormEvent) {
    ...
    try {
      if (editando) {
        await apiFetch(`/admin/motivos-declinio/${encodeURIComponent(editando)}`, {
          method: "PATCH",
          token,
          body: { nome },
        });
      } else {
        await apiFetch("/admin/motivos-declinio", { method: "POST", token, body: { nome } });
      }
      setEditando(null);
      setNome("");
      await load();
  ...
  // motivo alvo; esta função executa a inativação (soft-delete no backend) ao confirmar.
  async function confirmarInativacao() {
    ...
    setError(null);
    try {
      await apiFetch(`/admin/motivos-declinio/${encodeURIComponent(c.id)}`, {
        method: "DELETE",
        token,
      });
  ...
  async function reativar(c: Motivo) {
    try {
      await apiFetch(`/admin/motivos-declinio/${encodeURIComponent(c.id)}/reativar`, {
        method: "PATCH",
        token,
      });
      await load();
  ...
  return (
    <>
      ...
      <GlassCard as="form" onSubmit={salvar} className="mb-5 flex flex-wrap gap-3 p-4">
        {editando && (
          <p className="w-full text-sm text-accent">Editando um motivo, ajuste o nome e salve.</p>
        )}
        <input
          required
          placeholder={editando ? "Nome do motivo *" : "Novo motivo *"}
          value={nome}
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 text-sm">
        ...
        <input
          ...
          placeholder="Buscar motivo por nome"
          aria-label="Buscar motivo por nome"
