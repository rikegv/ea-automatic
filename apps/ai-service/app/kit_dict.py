...
from app.kit_motor import TituloKit


def carregar_dicionario_kit(kit_tipo_id: str) -> list[TituloKit]:
    """Títulos ATIVOS do kit, ordenados pela `ordem` do painel. Lista vazia se o kit não existe.

    Traz também o `padrao` (documento de instrução geral, que não é cobrado por nome). A marcação é
    CADASTRO, feita na tela de administração, nunca dedução pelo texto do título.
    """
    ...
    with psycopg.connect(url, autocommit=True) as conn:
        with conn.cursor() as cur:
            ...
            return [TituloKit(titulo=row[0], padrao=bool(row[1])) for row in cur.fetchall()]
