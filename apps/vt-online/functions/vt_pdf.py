"""Composicao dos documentos de VT: replica fiel do ai-service vt_pdf.py do EA.

Dois documentos distintos, ambos com cabecalho/logo Soulan:
  - OPTANTE: dados pessoais + itinerario de IDA + itinerario de VOLTA + total do dia + compromisso.
  - NAO-OPTANTE: declaracao de recusa com o texto aprovado pelo diretor.

O documento e DESENHADO a partir de dados estruturados, entao usa reportlab/platypus. Os bytes sao
devolvidos ao chamador e nada e gravado em disco aqui; nome/CPF nunca sao logados (LGPD).

Os textos apresentaveis ao usuario preservam os acentos do original do EA (fidelidade de conteudo).
Nenhum travessao (U+2014) e usado, conforme a regra permanente do projeto.
"""
...
# Paleta da marca Soulan (lida do proprio logo): azul do texto e verde do simbolo.
...
def _brl(valor: float) -> str:
    """R$ no padrao pt-BR (1.234,56). Gratuidade sai como R$ 0,00, que e a tarifa real."""
...
def _cabecalho(est: dict, titulo: str) -> list:
    """Cabecalho comum: logo Soulan + titulo do documento."""
    blocos: list = []
    if LOGO.exists():
        # 655x173 no original: mantem a proporcao ao fixar a largura.
        largura = 52 * mm
...
def _faixa_secao(est: dict, texto: str) -> Table:
    """Faixa azul de titulo de secao, largura total."""
...
def _tabela_itinerario(est: dict, conducoes: list, total: float) -> Table:
    """Colunas oficiais: meio de transporte | cartao/tipo | valor unitario (decisao do diretor)."""
    dados = [[
...
def _assinatura(est: dict) -> list:
    """Linha de assinatura + data. A assinatura em si e digital (Clicksign, etapa 3).

    Vai dentro de um KeepTogether: sem isso, um itinerario longo empurra so a linha de assinatura
    para a pagina seguinte e o documento e assinado numa folha solta, separada do que se declara.
    """
...
def gerar_optante(d: dict) -> bytes:
    """Documento do OPTANTE: itinerario de ida e volta, total do dia e compromisso."""
    est = _estilos()
    buf = BytesIO()
